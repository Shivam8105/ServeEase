...
export default function CustomerDashboard() {
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      // Mark the booking as updated locally or in DB if needed (optional)
      // Since booking was completed, we can just flag it. We'll set reviewStatus: "submitted"
                            ...
                            <td style={{ textAlign: "right" }}>
                              <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px" }}>
                                {booking.paymentStatus === "pending" && booking.status !== "cancelled" && (
                                  <Link href={`/checkout/${booking.id}`} className="btn btn-primary btn-sm">
                                    Pay Now
                                  </Link>
                                )}
